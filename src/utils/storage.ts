...
export const PREFIX = 'thetis_market'
...
/**
 * Will return `null` if browser storage is unavailable (like running app
...
Storage APIs are not supposed to be called on the server-rendering process.
 ...
 */
export function createStorageSlot(key: string, options?: { persistence?: StorageType }): StorageSlot {
  key = `${PREFIX}:${key}`
  if (typeof window === 'undefined') {
 ...
 */
export function listStorageKeys(storageType: StorageType = DefaultStorageType): string[] {
...
export const getStorageData = <T>(storageSlot: StorageSlot): T | undefined => {
  try {
    const data = storageSlot.get()
    if (!data) {
      return
    }
...
export const setStorageData = (storageSlot: StorageSlot, values: unknown) => {
  storageSlot.set(JSON.stringify(values))
}
