...
import { DEFAULT_CHAIN_ID, SWAP_ROUTER } from "@clmm/config";
import { ApprovalState, ApprovalStateType } from "@clmm/types/approve-state";

import { useNeedAllowance } from "./useNeedAllowance";
import { useTransactionAwait } from "./useTransactionAwait";
import { TransactionType } from "@clmm/state/pendingTransactionsStore.ts";
...
export function useApproveCallbackFromTrade(
    ...
    /* Compute maximumAmountIn for any trade — including native input. useApprove()
     * short-circuits to APPROVED when the currency is native; if we leave this
     * undefined for native, the early `!amountToApprove` check returns UNKNOWN
     * and the swap callback is gated off. */
