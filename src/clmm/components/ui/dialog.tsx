...
const DialogPortal = ({ children, ...props }: React.ComponentProps<typeof DialogPrimitive.Portal>) => (
    <DialogPrimitive.Portal {...props}>
        <div className="clmm-root" style={{ display: "contents" }}>{children}</div>
...
>(({ className, children, ...props }, ref) => (
    <DialogPortal>
        ...
        <DialogPrimitive.Content
            ref={ref}
            className={cn(
                "fixed z-50 grid w-full max-w-lg gap-4 rounded-xl border border-card-border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg md:w-full",
                className
            )}
            {...props}
            // Center via inline styles so it doesn't depend on Tailwind's
            // translate utilities / cascade-layer ordering (the CLMM CSS is
            // flattened for scoping, which broke the class-based centering).
            style={{ left: "50%", top: "50%", translate: "none", transform: "translate(-50%, -50%)", ...(props as { style?: React.CSSProperties }).style }}
        >
            {children}
