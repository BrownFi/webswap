...
// Hermes endpoint directly. Berachain and Robinhood Chain use BrownFi's BE proxy (Manh) which
...
// Deriving from VITE_API_URL keeps each chain on the right host per environment.
...
const API_PROXY_UPDATES = `${API_BASE}/pyth/v2/updates/price/latest`

/** Base URL for the price-update-blob + parsed-price fetch, for the given chain.
 *  Berachain + Robinhood + Arbitrum + Linea + HyperEVM use BrownFi's BE proxy
 *  (same response shape as Hermes). */
