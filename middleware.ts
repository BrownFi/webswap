// Vercel Edge Middleware — password gate for the V2-only LP-exit deployment.
//
// Runs at the edge on every request BEFORE any asset is served, so the key
// never reaches the browser bundle (unlike VITE_* vars, which are public).
// Unauthenticated visitors get a branded login PAGE; a correct key sets an
// httpOnly cookie and the normal app loads.
//
// This is the Vercel port of the beta Cloudflare Pages gate
// (functions/_middleware.js). Same password + same double-hash scheme, so the
// beta access key works here unchanged. The two differ only in host plumbing:
//   - CF Pages Function:  onRequest(context) + context.next()
//   - Vercel Middleware:  default export(request) + next() (x-middleware-next)
//
// KEY SOURCE (priority): process.env.GATE_KEY (a Vercel project env var, plaintext)
// -> else the committed KEY_CHECK double-hash below. Set GATE_KEY in the Vercel
// dashboard to rotate the key without committing anything; leave it unset to use
// the shared beta key baked into KEY_CHECK.
//
// LOCAL TEST: `npx vercel dev` (or `vercel build` + `vercel dev`). Plain
// `vite`/`yarn dev` does NOT execute Vercel middleware.

export const config = {
  // Gate every route (same blanket coverage as the beta CF _middleware). Static
  // assets are harmless and public, but gating everything keeps the app document
  // itself protected and mirrors beta exactly.
  matcher: '/:path*',
}

// Active access check = SHA-256( SHA-256('bf-gate-v1:' + key) ) of the key. The
// plaintext key is NOT stored here (the repo is public) — only this double hash,
// from which neither the key nor a valid session cookie can be derived. To change
// it: recompute sha256Hex(tokenFor(newKey)), or set the GATE_KEY env var
// (plaintext) in the Vercel dashboard to override. Current key: 'v2brownfi2026'.
const KEY_CHECK = '4ee8ca6ad7690b05087469182faabeacfd82c0286707dc1de8658034e1662e29'

const COOKIE = 'bf_gate'
const MAX_AGE = 60 * 60 * 24 * 30 // 30 days
const LOGO = 'data:image/svg+xml;base64,'

async function sha256Hex(str: string): Promise<string> {
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(str))
  return [...new Uint8Array(buf)].map((b) => b.toString(16).padStart(2, '0')).join('')
}

// Session-cookie value = SHA-256 of the key (h1). A cookie is valid iff
// SHA-256(cookie) === KEY_CHECK, so the committed KEY_CHECK (the double hash)
// can't be used to forge a cookie, and the plaintext key is never stored. httpOnly.
const tokenFor = (key: string) => sha256Hex('bf-gate-v1:' + key)

function parseCookies(header: string): Record<string, string> {
  const out: Record<string, string> = {}
  header.split(';').forEach((part) => {
    const i = part.indexOf('=')
    if (i > 0) out[part.slice(0, i).trim()] = part.slice(i + 1).trim()
  })
  return out
}

function loginPage(error: boolean): string {
  const err = error ? '<div class="error">Incorrect key — please try again.</div>' : ''
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1" />
<meta name="robots" content="noindex,nofollow" />
<title>BrownFi · Access</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;color:#CFC7C1;background:radial-gradient(1100px 560px at 50% -8%,#2A211A 0%,#171210 55%,#100D0B 100%)}
  .card{width:100%;max-width:384px;background:rgba(30,25,21,.82);border:1px solid #2F2823;border-radius:18px;padding:38px 32px;box-shadow:0 28px 70px rgba(0,0,0,.5);backdrop-filter:blur(6px);text-align:center}
  .logo{height:30px;margin-bottom:22px;opacity:.97}
  .badge{display:inline-block;font-size:10.5px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#E0AE80;background:rgba(152,92,42,.16);border:1px solid rgba(152,92,42,.42);padding:4px 11px;border-radius:999px;margin-bottom:18px}
  h1{font-size:18px;font-weight:600;color:#FBFBFD;margin-bottom:7px}
  p.sub{font-size:13px;color:#978A80;line-height:1.55;margin-bottom:24px}
  form{display:flex;flex-direction:column;gap:12px}
  .error{font-size:12.5px;color:#EE8A79;background:rgba(224,72,72,.1);border:1px solid rgba(224,72,72,.28);border-radius:9px;padding:9px 11px}
  input{width:100%;padding:13px 14px;font-size:14px;font-family:inherit;background:#161210;border:1px solid #3A302A;border-radius:11px;color:#FBFBFD;outline:none;transition:border-color .15s,box-shadow .15s}
  input::placeholder{color:#6B5F56}
  input:focus{border-color:#985C2A;box-shadow:0 0 0 3px rgba(152,92,42,.18)}
  button{width:100%;padding:13px;font-size:14px;font-weight:600;font-family:inherit;color:#fff;border:none;border-radius:11px;cursor:pointer;background:linear-gradient(180deg,#AC6A31 0%,#8A4F22 100%);transition:filter .15s}
  button:hover{filter:brightness(1.08)}
  .pw{position:relative;display:flex}
  .pw input{padding-right:44px}
  .toggle{position:absolute;top:50%;right:5px;transform:translateY(-50%);width:34px;height:34px;padding:0;background:transparent;border:none;border-radius:8px;color:#8A7D72;display:flex;align-items:center;justify-content:center;cursor:pointer;filter:none}
  .toggle:hover{color:#D8A072;background:transparent;filter:none}
  .foot{margin-top:22px;font-size:11px;color:#6B5F56;letter-spacing:.02em}
</style>
</head>
<body>
  <div class="card">
    <img class="logo" src="${LOGO}" alt="BrownFi" />
    <div class="badge">V2 · LP access</div>
    <h1>Restricted access</h1>
    <p class="sub">This BrownFi V2 interface is for existing liquidity providers. Enter the access key provided by the BrownFi team to continue.</p>
    <form method="POST" action="/__gate">
      ${err}
      <div class="pw">
        <input id="key" type="password" name="key" placeholder="Access key" autocomplete="off" autofocus required />
        <button type="button" class="toggle" id="toggle" aria-label="Show password" title="Show / hide"></button>
      </div>
      <button type="submit">Unlock</button>
    </form>
    <div class="foot">Access provided by the BrownFi team</div>
  </div>
  <script>
    (function () {
      var EYE = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
      var OFF = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';
      var inp = document.getElementById('key'), btn = document.getElementById('toggle');
      btn.innerHTML = EYE;
      btn.addEventListener('click', function () {
        var show = inp.type === 'password';
        inp.type = show ? 'text' : 'password';
        btn.innerHTML = show ? OFF : EYE;
        btn.setAttribute('aria-label', show ? 'Hide password' : 'Show password');
        inp.focus();
      });
    })();
  </script>
</body>
</html>`
}

function htmlResponse(html: string, status: number): Response {
  return new Response(html, {
    status,
    headers: { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' },
  })
}

// Continue to the origin (serve the static SPA). This is exactly what
// @vercel/functions `next()` returns — a null-body Response carrying the
// x-middleware-next signal — inlined so the gate needs no extra dependency
// (the repo's committed lockfile is package-lock.json).
function next(): Response {
  return new Response(null, { headers: { 'x-middleware-next': '1' } })
}

export default async function middleware(request: Request): Promise<Response> {
  // Expected double hash: derived from GATE_KEY (Vercel project env var) if set,
  // else the committed KEY_CHECK (the shared beta key). Empty -> gate disabled.
  const envKey = typeof process !== 'undefined' ? process.env.GATE_KEY : undefined
  const check = envKey ? await sha256Hex(await tokenFor(envKey.trim())) : KEY_CHECK
  if (!check) return next()

  const url = new URL(request.url)

  // Already authenticated: cookie holds h1, valid iff SHA-256(h1) === check.
  const cookies = parseCookies(request.headers.get('Cookie') || '')
  const cookieVal = cookies[COOKIE]
  if (cookieVal && (await sha256Hex(cookieVal)) === check) return next()

  // Login submission.
  if (request.method === 'POST' && url.pathname === '/__gate') {
    const form = await request.formData()
    const submitted = (form.get('key') || '').toString().trim()
    const token = await tokenFor(submitted)
    if ((await sha256Hex(token)) === check) {
      const secure = url.protocol === 'https:' ? '; Secure' : ''
      return new Response(null, {
        status: 303,
        headers: {
          'Set-Cookie': `${COOKIE}=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${MAX_AGE}${secure}`,
          Location: '/',
          'cache-control': 'no-store',
        },
      })
    }
    return htmlResponse(loginPage(true), 401)
  }

  // Anything else while unauthenticated -> the login page.
  return htmlResponse(loginPage(false), 200)
}
