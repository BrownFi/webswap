...
var Pair = /*#__PURE__*/function () {
  ...
  _proto.getOutputAmountAsync = function getOutputAmountAsync(inputAmount, path, chainId, pairAddress, account) {
    try {
      ...
      return Promise.resolve(Promise.all([pairContract.methods.priceFeed().call(), pairContract.methods.qti().call()])).then(function (_ref) {
        ...
        function _temp2() {
          var _balances;
          var outputAmount = new TokenAmount(inputAmount.token.equals(_this.token0) ? _this.token1 : _this.token0, (_balances = balances) === null || _balances === void 0 ? void 0 : _balances[1]);
          if (JSBI.equal(outputAmount.raw, ZERO)) {
            throw new InsufficientInputAmountError();
          }
          return [outputAmount, new Pair(inputReserve.add(inputAmount), outputReserve.subtract(outputAmount)), 0, +qti, priceFeedUpdateData, updateFee];
        }
        ...
        var _temp = function () {
          if (supportContractWithPrice(chainId)) {
            ...
            return Promise.resolve(Promise.all([pythPriceFeedContract.methods.baseTokenPriceId().call(), pythPriceFeedContract.methods.quoteTokenPriceId().call()])).then(function (_ref2) {
              var baseTokenPriceId = _ref2[0],
                quoteTokenPriceId = _ref2[1];
              return Promise.resolve(getPriceFeedUpdateData(baseTokenPriceId, quoteTokenPriceId)).then(function (_getPriceFeedUpdateDa) {
                ...
                return Promise.resolve(pythContract.methods.getUpdateFee(priceFeedUpdateData).call()).then(function (_pythContract$methods) {
                  ...
                  return Promise.resolve(routerContractWithPrice.methods.getAmountsOutWithPrice(inputAmount.raw.toString(), path === null || path === void 0 ? void 0 : path.map(function (token) {
                    ...
                    balances = _routerContractWithPr;
                  });
                });
              });
            });
            ...
            return Promise.resolve(routerContract.methods.getAmountsOut(inputAmount.raw.toString(), path === null || path === void 0 ? void 0 : path.map(function (token) {
              ...
              balances = _routerContract$metho;
            });
          }
        }();
  ...
  _proto.getInputAmountAsync = function getInputAmountAsync(outputAmount, path, chainId, pairAddress, account) {
    try {
      ...
      var routerContractWithPrice = new web3.eth.Contract(IRouterWithPrice, ROUTER_ADDRESS_WITH_PRICE[chainId]);
      var pairContract = new web3.eth.Contract(IPair, pairAddress);
      return Promise.resolve(Promise.all([pairContract.methods.priceFeed().call(), pairContract.methods.qti().call()])).then(function (_ref3) {
        var priceFeedAddress = _ref3[0],
          qti = _ref3[1];
        function _temp4() {
          return Promise.resolve(Promise.all([supportContractWithPrice(chainId) ? routerContractWithPrice.methods.getAmountsInWithPrice(outputAmount.raw.toString(), path === null || path === void 0 ? void 0 : path.map(function (token) {
            ...
            return token.address;
          })).call(), !supportContractWithPrice(chainId) && priceFeedContract.methods.latestRoundData().call(), !supportContractWithPrice(chainId) && priceFeedContract.methods.decimals().call(), supportContractWithPrice(chainId) && getLatestPrice(basePriceIds, quotePriceIds)])).then(function (_ref4) {
            var _latestPrice$parsed;
            var balances = _ref4[0],
              latestRound = _ref4[1],
              decimals = _ref4[2],
              latestPrice = _ref4[3];
            var basePrice = 1;
        ...
        var _temp3 = function () {
          if (supportContractWithPrice(chainId)) {
            var pythPriceFeedContract = new web3.eth.Contract(IPythPriceFeed, priceFeedAddress);
            return Promise.resolve(Promise.all([pythPriceFeedContract.methods.baseTokenPriceId().call(), pythPriceFeedContract.methods.quoteTokenPriceId().call()])).then(function (_ref5) {
              var baseTokenPriceId = _ref5[0],
                quoteTokenPriceId = _ref5[1];
    ...
    this.init = function (route, amount, tradeType, account) {
      try {
        var _temp4 = function _temp4() {
          ...
          var reserve = +reserveOut.raw / Math.pow(10, reserveOut.currency.decimals);
          _this.priceImpactK = K * amountOut / reserve * 100;
          ...
          if (tradeType === TradeType.EXACT_INPUT) {
            ...
            var _temp = _for(function () {
              ...
              return Promise.resolve(pair.getOutputAmountAsync(amounts[_i], route.path, pair.chainId, pair.liquidityToken.address, account)).then(function (_ref) {
                ...
                reserveOut = isForward ? nextPair.reserve0 : nextPair.reserve1;
