...
// Active dev access check = SHA-256( SHA-256('bf-gate-v1:' + key) ) of the dev
// key. The plaintext key is NOT stored here (the repo is public) — only this
// double hash, from which neither the key nor a valid session cookie can be
// derived. To change the key: recompute this for the new key (sha256Hex of
// tokenFor(newKey)), commit, push. Or set BETA_ACCESS_KEY (plaintext) as a CF
// dashboard env var to override without committing anything.
const KEY_CHECK = 'e74f4d57d813e0f957433116a05435fcda3bad2d5aacc0837965497bfeea47bb'
...
const tokenFor = (key) => sha256Hex('bf-gate-v1:' + key)
...
export async function onRequest(context) {
  ...
  // Expected double hash: derived from BETA_ACCESS_KEY (CF dashboard env var) if
  // set, else the committed KEY_CHECK. Empty -> gate disabled, pass through.
  const check = env.BETA_ACCESS_KEY ? await sha256Hex(await tokenFor(env.BETA_ACCESS_KEY.trim())) : KEY_CHECK
