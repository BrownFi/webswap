...
export async function onRequest(context) {
  ...
  let body
  try { body = await context.request.json() } catch { return Response.json({ pools: [] }, { status: 400 }) }
  ...
  if (chainId !== 4663 || !Array.isArray(poolIdentifiers) || poolIdentifiers.length > 30 || normalizedPoolIds.length !== poolIdentifiers.length || normalizedPoolIds.some((id) => !ALLOWED_POOL_IDS_LOWER.has(id))) return Response.json({ pools: [] }, { status: 400 })
