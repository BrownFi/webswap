...
export default async function handler(req, res) {
  ...
  if (chainId !== CHAIN_ID || !Array.isArray(poolIdentifiers) || poolIdentifiers.length > 30 || poolIdentifiers.some((id) => typeof id !== 'string' || !ALLOWED_POOL_IDS.has(id))) return res.status(400).json({ pools: [] })
  ...
  const body = { pools }
  cache.set(cacheKey, { body, expiresAt: Date.now() + CACHE_TTL })
  return res.status(200).setHeader('cache-control', 'public, max-age=60').json(body)
}
